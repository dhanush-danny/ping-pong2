...
function preload(){
  ball_paddle_touch=loadSound("ball_touch_paddel.wav");
...
function get_result(results_array) {
  if (results_array.length > 0) {
    ...
    right_w_x=results_array[0].pose.rightWrist.x;
    right_w_score=results_array[0].pose.keypoints[10].score;
    right_w_y=results_array[0].pose.rightWrist.y;
  console.log(right_w_score);
  }
}
...
function move() {
  ...
  if (pcscore == 4) {
    ...
    text("Press Reset Button To Play The Game Again!", width / 2, height / 2 + 30)
    noLoop();
    pcscore = 0;
  }
